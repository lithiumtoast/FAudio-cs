...
function build_faudio() {
    ...
    cmake -S $DIR/ext/FAudio -B $FAUDIO_BUILD_DIR $CMAKE_ARCH_ARGS \
        `#Change output directories` \
        -DCMAKE_ARCHIVE_OUTPUT_DIRECTORY=$FNA3D_BUILD_DIR -DCMAKE_LIBRARY_OUTPUT_DIRECTORY=$FAUDIO_BUILD_DIR -DCMAKE_RUNTIME_OUTPUT_DIRECTORY=$FAUDIO_BUILD_DIR -DCMAKE_RUNTIME_OUTPUT_DIRECTORY_RELEASE=$FAUDIO_BUILD_DIR \
        `# project specific` \
        -DSDL2_INCLUDE_DIRS="$SDL_INCLUDE_DIRECTORY_PATH" -DSDL2_LIBRARIES="$SDL_LIBRARY_FILE_PATH"
    exit_if_last_command_failed
    ...
    if [[ "$TARGET_BUILD_OS" == "macos" ]]; then
        install_name_tool -delete_rpath "$LIB_DIR"
    fi
